...
```{r}
pitches = read.csv(file = "C:\\Users\\Leigh\\OneDrive\\Documents\\Saberseminar Pitches\\data\\pitches.csv")

atbats = read.csv(file = "C:\\Users\\Leigh\\OneDrive\\Documents\\Saberseminar Pitches\\data\\atbats.csv")

players = read.csv(file = "C:\\Users\\Leigh\\OneDrive\\Documents\\Saberseminar Pitches\\data\\player_names.csv")
```
...
```{r}
saber1$inning = as.integer(saber1$inning)
...
# comparing starter models
```{r}
AIC(model.1.2, model.2.2, model.3.2, model.4,  model.5, model.6, model.7, model.8, model.9)
```

# final starter model plot
```{r}
hundred = data.frame(pitches = 1:500)

ggplot(hundred,aes(pitches)) +
 stat_function(fun=function(x) (-0.03987 + 0.000001498*(x^2)))

# intercept: -3.987e-02
# pitch_count: - 4.526e-04
# pitch_count*2 estimate: 1.498e-06


ggplot(hundred,aes(pitches)) +
 stat_function(fun=function(x) (-0.03987 - 0.0004526*x + 0.000001498*(x^2)))
```
...
# reliever model
```{r}
fifty = data.frame(pitches = 1:50)

# intercept = -3.684e-02
# linear estimate = 5.775e-05
# quadratic estimate = -3.754e-06


ggplot(hundred,aes(pitches)) +
 stat_function(fun=function(x) (-0.03684 + 0.00005775*x - 0.000003754*(x^2)))
```
